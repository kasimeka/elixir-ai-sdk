defmodule AI.Providers.OpenAI.ChatLanguageModel do
  ...
  def do_generate(%__MODULE__{} = model, options) do
    {body, warnings} = get_args(model, options)

    url = get_url(model, "/chat/completions")
    headers = get_headers(model, options)

    case JsonApi.post(url, body, headers, options) do
      {:ok, response} ->
  ...
  def do_stream(%__MODULE__{} = model, options) do
    ...
    case EventSource.post(url, body, headers, options) do
      {:ok, response} ->
  ...
  defp get_args(model, %{
    ...
    warnings = []
    warnings = if top_k != nil, do: [%{type: :unsupported_setting, setting: :top_k} | warnings], else: warnings
    ...
    chat_messages = ChatMessages.convert_to_openai_chat_messages(
      prompt,
    ...
    args = case mode.type do
      :regular ->
        {tools, tool_warnings} = prepare_tools(mode, use_legacy_function_calling, supports_structured_outputs?(model))
        {args, warnings} = {Map.merge(base_args, tools), warnings ++ tool_warnings}
        {args, warnings}

      :object_json ->
        response_format = if supports_structured_outputs?(model) && mode.schema != nil do
          %{
            type: "json_schema",
            json_schema: %{
              schema: mode.schema,
  ...
  defp maybe_adjust_reasoning_model_settings(args, model, warnings) do
    cond do
      is_reasoning_model?(model.model_id) ->
        ...
        args
        |> Map.drop([:temperature, :top_p, :frequency_penalty, :presence_penalty, :logit_bias, :logprobs, :top_logprobs])
  ...
  defp add_unsupported_setting_warnings(warnings, args) do
    warnings
    |> maybe_add_warning(args[:temperature], :temperature, "temperature is not supported for reasoning models")
    |> maybe_add_warning(args[:top_p], :top_p, "topP is not supported for reasoning models")
    |> maybe_add_warning(args[:frequency_penalty], :frequency_penalty, "frequencyPenalty is not supported for reasoning models")
    |> maybe_add_warning(args[:presence_penalty], :presence_penalty, "presencePenalty is not supported for reasoning models")
    |> maybe_add_warning(args[:logit_bias], :other, "logitBias is not supported for reasoning models")
    |> maybe_add_warning(args[:logprobs], :other, "logprobs is not supported for reasoning models")
    |> maybe_add_warning(args[:top_logprobs], :other, "topLogprobs is not supported for reasoning models")
  end
  ...
  defp handle_generate_response(_response, body, warnings) do
    # TODO: Implement response handling
    {:ok, %{text: "Hello, world!", finish_reason: "stop", usage: %{prompt_tokens: 10, completion_tokens: 20, total_tokens: 30}, raw_call: body, warnings: warnings}}
  end
